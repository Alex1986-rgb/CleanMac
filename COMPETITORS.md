...
- [~] **Sunburst-карта диска** с drill-down (в работе) (превратить текущую карту в интерактивное кольцо). `medium`
- [ ] **«Scan as admin»** — показ скрытого системного места (реальные десятки ГБ). `medium`
- [x] **Меню-бар спутник** (`menubar.py`/rumps): живые CPU/RAM/SWAP/температура + кнопка Smart Care. `medium`
- [ ] **Термо-дашборд + бенчмарк диска** (powermetrics/SMC, read/write тест). `easy→medium`
- [~] **Лёгкий adware-скан** (в работе) по известным путям LaunchAgents (не полноценный AV). `medium`
...
- [x] **Health Report** — итог одним экраном после сканирования + экспорт HTML/PDF. `easy`
- [x] **Похожие изображения** (perceptual hash) ✅ `medium`
- [x] **Битые/нулевые файлы, broken symlinks** ✅ `easy→medium`
- [~] **Обратимый «Focus Mode»** (в работе) — пауза фоновых процессов с авто-восстановлением (безопаснее kill). `medium`
- [x] **Менеджер расширений браузеров** + улучшенная приватность по профилям. `medium`
- [ ] **Реестр Windows — только бэкап/снапшот веток** перед изменениями (НЕ «чистка»). `medium`
- [x] **Защита от over-cleaning**: whitelist рискованных кэшей + dry-run превью. `easy`
...
- [x] **Swipe-разбор фото** ✅ («one-by-one», undo, счётчик «освобождено N МБ»). `medium`
- [x] **Точный прогноз освобождаемого места** ✅ до системного диалога удаления. `easy`
- [ ] **Honest-onboarding** (реальный скан → мягкий paywall после первой пользы). `medium`
- [~] **Smart-категории медиа** (частично) (GIF/мемы, Live Photos, «давно не открывал»). `easy→medium`
- [x] **Старые события календаря** (EventKit) ✅ `easy`
- [x] **Виджеты + честные локальные уведомления (Автопилот)** (без «storage full» страшилок). `medium`
...
- [x] **Smart-подсказки на дашборде** ✅ (Files-style карточки: кэш, похожие фото, осиротевшие, давно не открывал). `medium`
- [x] **Корзина / undo** (createTrashRequest) ✅ (Android 11+). `easy→medium`
- [~] **Похожие/размытые фото** (perceptual hash + Лапласиан) — в работе
- [~] **CorpseFinder-lite** (в работе) — осиротевшие файлы удалённых приложений (MediaStore × список пакетов). `medium`
- [x] **«Неиспользуемые приложения»** (UsageStatsManager). `medium`
- [x] **Планировщик авто-сканов + уведомления (Автопилот, WorkManager)** ✅ `medium`
